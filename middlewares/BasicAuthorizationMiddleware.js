...
export const protectRoute = async(req, res, next) => {
    ...
    try {
        ...
        if (!authHeader || !authHeader.startsWith('Basic ')) {
            errorLogger.error('Unauthorized: Invalid or Missing Authorization Header')
            res.status(401).json({ message: "Unauthorized: Invalid or Missing Authorization Header"}).end()
            return res
        }
